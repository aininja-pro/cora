...
CREATE INDEX idx_property_inquiries_call_id ON property_inquiries(call_id);
CREATE INDEX idx_lead_capture_phone ON lead_capture(phone_number);
CREATE INDEX idx_lead_capture_status ON lead_capture(lead_status);
CREATE INDEX idx_calls_phone ON calls(phone_number);
CREATE INDEX idx_calls_status ON calls(call_status);

-- Add trigger for lead_capture updated_at
