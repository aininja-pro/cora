-- COMPLETE CORA SUPABASE SCHEMA
-- Run this entire script in Supabase SQL Editor
-- This includes all tables needed for SMS notifications + existing CORA tables
...
CREATE POLICY IF NOT EXISTS tenants_service_full
ON tenants FOR ALL TO service_role
USING (true) WITH CHECK (true);

CREATE POLICY IF NOT EXISTS contacts_service_full
ON contacts FOR ALL TO service_role
USING (true) WITH CHECK (true);

CREATE POLICY IF NOT EXISTS calls_service_full
ON calls FOR ALL TO service_role
USING (true) WITH CHECK (true);

CREATE POLICY IF NOT EXISTS call_transcripts_service_full
ON call_transcripts FOR ALL TO service_role
USING (true) WITH CHECK (true);

CREATE POLICY IF NOT EXISTS notifications_service_full
ON notifications FOR ALL TO service_role
USING (true) WITH CHECK (true);

-- Authenticated users get tenant-scoped read access (for UI)
CREATE POLICY IF NOT EXISTS notifications_tenant_select
ON notifications FOR SELECT TO authenticated
USING (tenant_id = (auth.jwt() ->> 'tenant_id'));

CREATE POLICY IF NOT EXISTS calls_tenant_select  
...
WHERE schemaname = 'public' 
    AND tablename IN ('agents', 'listings', 'tasks', 'calls', 'voice_transcriptions', 
                     'tenants', 'contacts', 'call_transcripts', 'notifications')
ORDER BY tablename;